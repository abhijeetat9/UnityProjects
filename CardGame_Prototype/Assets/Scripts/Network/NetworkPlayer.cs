...
public class NetworkPlayer : NetworkBehaviour
{
    ...
    private NetworkGameController GetGameControllerFor(NetworkConnectionToClient sender)
    {
        CabbooNetworkManager cabbooNetworkManager = NetworkManager.singleton as CabbooNetworkManager;
        var room = cabbooNetworkManager.GetRoomForConnection(sender);
        if (room == null)
            return null;
        if(room.gameRoomInstance == null)
            return null;
        NetworkGameController networkGameController = room.gameRoomInstance.GetComponentInChildren<NetworkGameController>();
    ...
    public void CmdStartGame(NetworkConnectionToClient sender = null)
    {
        ...
        var result = cabbooNetworkManager.GetRoomForConnection(sender);
        
        if (result == null)
        {
            Debug.Log($"[StartGame] {sender}: No room found for {sender}");
            return;
            
        }
        if (result._hostConnection != sender)
        {
            Debug.LogWarning($"Connection {sender.connectionId} tried to start the game but is NOT the host of Room {result._inviteCode}.");
            return;
        }
        
        result.gameRoomInstance = Instantiate(cabbooNetworkManager.GameRoomPrefab);
        _roomInterestManagement.RegisterRoom(result.gameRoomInstance.GetComponent<NetworkIdentity>(), result);
        
        NetworkServer.Spawn(result.gameRoomInstance);
        var connectedPlayers = result.connections.Count;
        GameController gameController = new GameController(RoundSetup.CreateGameState(connectedPlayers)
        );
        NetworkGameController networkGameController = result.gameRoomInstance.GetComponentInChildren<NetworkGameController>();
        networkGameController.Initialize(gameController, result);
    }
    ...
    [Command]
    public void CmdDraw(NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
    ...
    [Command]
    public void CmdDiscard(NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdSwap(int slotIndex, NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdSwapFromDiscard(int slotIndex, NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdSkipNext(NetworkConnectionToClient sender  = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdPeekOwn(int slotIndex, NetworkConnectionToClient sender  = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdPeekTarget(int slotIndex,  int targetPlayerId, NetworkConnectionToClient sender  = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdBlindSwap(int slotIndex, int targetPlayerId, int targetSlotIndex, NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdLookAndSwapPeekOwn(int slotIndex, NetworkConnectionToClient sender  = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdLookAndSwapPeekTarget(int slotIndex, int targetPlayerId, NetworkConnectionToClient sender  = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdRedKingDecision(bool shouldSwap, int ownSlotIndex, NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdCallCabboo(NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return;
        var gameController = networkGameController.GameController;
    ...
    [Command]
    public void CmdNewRound(NetworkConnectionToClient sender = null)
    {
        var networkGameController = GetGameControllerFor(sender);
        if (networkGameController == null) return; 
        networkGameController.BeginNewRound(networkGameController.room.connections.Count);
    }
